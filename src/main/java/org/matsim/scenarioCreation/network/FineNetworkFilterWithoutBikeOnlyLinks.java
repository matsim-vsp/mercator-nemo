...
public class FineNetworkFilterWithoutBikeOnlyLinks implements OsmNetworkReader.OsmFilter {

    private final List<Geometry> geometries = new ArrayList<>();

    FineNetworkFilterWithoutBikeOnlyLinks(String pathToShapeFile) {
    ...
    public boolean coordInFilter(Coord coord, int hierarchyLevel) {
        ...
        return hierarchyLevel <= 8 && geometries.stream().anyMatch(geometry -> geometry.contains(MGC.coord2Point(coord)));
    }
}
